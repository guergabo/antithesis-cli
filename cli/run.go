...
antithesis run \
  ...
  --tenant='brick-whale' \
  --username='dogfood' \
  --password='6SRu$h!0f2fNZCZtMGyEg(krgaRA&R' \
  ...
  --email='gguergabo@gmail.com'`,
	...
	cmd.Flags().StringVarP(&name, "name", "n", "defaultvalue", "description")
	cmd.Flags().StringVarP(&description, "description", "d", "defaultvalue", "description")
	cmd.Flags().StringVarP(&notebook, "notebook", "", "basic_test", "description")
	cmd.Flags().StringVarP(&tenant, "tenant", "t", "defaultvalue", "description")
	cmd.Flags().StringVarP(&username, "username", "u", "defaultvalue", "description")
	cmd.Flags().StringVarP(&password, "password", "p", "defaultvalue", "description")
	cmd.Flags().StringVarP(&config, "config", "c", "defaultvalue", "config image url.")
	cmd.Flags().StringArrayVarP(&images, "image", "i", make([]string, 0), "images url.")
	cmd.Flags().Int32VarP(&duration, "duration", "", 15, "description")
	cmd.Flags().StringArrayVarP(&emails, "email", "e", make([]string, 0), "email recipients")
